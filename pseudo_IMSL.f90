...
!
    ...
    real(kind=dkind), dimension(nx), intent(in)        :: xvec
    real(kind=dkind), dimension(nx+kxord), intent(out) :: xknot
    ...
    if(mod(kxord,2) .eq. 0) then
       ...
       do ix = kxord+1, nx
          xknot(ix) = 0.5_dkind * (xvec(ix-kxord/2) + xvec(ix-kxord/2-1))
       end do
    endif

    do ix = nx+1, nx+kxord
       xknot(ix) = xvec(nx) * (1.0_dkind + eps)
    end do
...
!
    ...
    real(kind=dkind), dimension(nx), intent(in)    :: xdata, xvec
    real(kind=dkind), dimension(nx+kx), intent(in) :: xknot
    real(kind=dkind), dimension(nx), intent(out)   :: bcoef

    integer                                :: nxp1, kxm1, kpkm2, leftx, lenq
    integer                                :: ix, ik,ilp1mx, jj, iflag
    real(kind=dkind)                         :: xveci
    real(kind=dkind), dimension((2*kx-1)*nx) :: work
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	function dbsval_safe(x,kx,xknot,nx,bcoef,yknot) result(answer)
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    integer, intent(in)                          :: nx, kx
 !   real(kind=dkind)                               :: dbsval
    real(kind=dkind)                               :: x
    real(kind=dkind), dimension(nx+kx), intent(in) :: xknot
    real(kind=dkind), dimension(nx), intent(in)    :: bcoef
	real(kind=dkind), dimension(nx), intent(in)    :: yknot
	real(kind=dkind) :: answer

	if(x==xknot(nx+kx)) then

		answer = yknot(nx)

	else

		answer = dbsval(x,kx,xknot,nx,bcoef)
...
!
    ...
    real(kind=dkind)                :: save1, save2
    real(kind=dkind), dimension(kx) :: work, dl, dr
...
!
    ...
    real(kind=dkind)                               :: dbsder
    real(kind=dkind), intent(in)                   :: x
    real(kind=dkind), dimension(nx+kx), intent(in) :: xknot
    real(kind=dkind), dimension(nx), intent(in)    :: bcoef

    integer                       :: ix, ik, il, leftx
    real(kind=dkind)                :: save, save1, save2, y, sum, dik
    real(kind=dkind), dimension(kx) :: work, dl, dr,bsp
...
!
    ...
    real(kind=dkind), dimension(nx), intent(in)     :: xvec
    real(kind=dkind), dimension(ny), intent(in)     :: yvec
    real(kind=dkind), dimension(nx+kx), intent(in)  :: xknot
    real(kind=dkind), dimension(ny+ky), intent(in)  :: yknot
    real(kind=dkind), dimension(ldf,*), intent(in)  :: xydata
    real(kind=dkind), dimension(nx,ny), intent(out) :: bcoef

    real(kind=dkind), dimension(max(nx,ny),max(nx,ny))        :: work1
    real(kind=dkind), dimension(max(nx,ny))                   :: work2
    real(kind=dkind), dimension(max((2*kx-1)*nx,(2*ky-1)*ny)) :: work3
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    real(kind=dkind), dimension(n), intent(in)    :: xyvec
    real(kind=dkind), dimension(n+k), intent(in)  :: xyknot
    real(kind=dkind), dimension(ld,m), intent(in) :: xydata
    real(kind=dkind), dimension(m,n), intent(out) :: bcoef

    real(kind=dkind), dimension(n), intent(out)         :: work2
    real(kind=dkind), dimension((2*k-1)*n), intent(out) :: work3


    integer        :: np1, km1, kpkm2, left, lenq, i, iflag, ilp1mx, j, jj
    real(kind=dkind) :: xyveci
...
!
    ...
    real(kind=dkind), dimension(nx+kx), intent(in) :: xknot
    real(kind=dkind), dimension(ny+ky), intent(in) :: yknot
    real(kind=dkind), dimension(nx,ny), intent(in) :: bcoef
    real(kind=dkind)                               :: dbs2vl

    integer                       :: ix, iy, iky, leftx, lefty
    real(kind=dkind), dimension(ky) :: work
...
!
    ...
    real(kind=dkind), dimension(nx), intent(in)         :: xvec
    real(kind=dkind), dimension(ny), intent(in)         :: yvec
    real(kind=dkind), dimension(nz), intent(in)         :: zvec
    real(kind=dkind), dimension(nx+kx), intent(in)      :: xknot
    real(kind=dkind), dimension(ny+ky), intent(in)      :: yknot
    real(kind=dkind), dimension(nz+kz), intent(in)      :: zknot
    real(kind=dkind), dimension(ldf,mdf,nz), intent(in) :: xyzdata
    real(kind=dkind), dimension(nx,ny,nz), intent(out)  :: bcoef

    integer                                :: iz
    real(kind=dkind), dimension(nx,ny,nz)    :: work1
    real(kind=dkind), dimension(nz)          :: work2
    real(kind=dkind), dimension((2*kz-1)*nz) :: work3
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    real(kind=dkind), dimension(n), intent(in)          :: xyzvec
    real(kind=dkind), dimension(n+k), intent(in)        :: xyzknot
    real(kind=dkind), dimension(ldf,mdf,*), intent(in)  :: xyzdata
    real(kind=dkind), dimension(nx,ny,nz), intent(out)  :: bcoef
    real(kind=dkind), dimension(n), intent(out)         :: work2
    real(kind=dkind), dimension((2*k-1)*n), intent(out) :: work3

    integer        :: np1, km1, kpkm2, left, lenq, i, ilp1mx, j, jj, iflag, in
    real(kind=dkind) :: xyzveci
...
!
    ...
    integer, intent(in)                             :: nx, ny, nz, kx, ky, kz
    real(kind=dkind), intent(in)                      :: x, y, z
    real(kind=dkind), dimension(nx+kx), intent(in)    :: xknot
    real(kind=dkind), dimension(ny+ky), intent(in)    :: yknot
    real(kind=dkind), dimension(nz+kz), intent(in)    :: zknot
    real(kind=dkind), dimension(nx,ny,nz), intent(in) :: bcoef
    real(kind=dkind)                                  :: dbs3vl

    integer                       :: iz, nintz
    real(kind=dkind), dimension(kz) :: work
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    real(kind=dkind), intent(in)                    :: x
    real(kind=dkind), dimension(n), intent(in)      :: t
    real(kind=dkind), dimension(jhigh), intent(out) :: biatx

    integer                          :: j = 1
    integer                          :: i, jp1
    real(kind=dkind)                   :: saved, term
    real(kind=dkind), dimension(jhigh) :: dl, dr
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    real(kind=dkind), dimension(nroww,nrow), intent(inout) :: w

    real(kind=dkind) :: pivot, factor
...
10  if (nbandl .gt. 0) go to 30
    ...
    do i = 1, nrowm1
       if (w(middle,i) .eq. 0._dkind) go to 999
    end do
...
30  if (nbandu .gt. 0) go to 60
    ...
    do i = 1, nrowm1
       pivot = w(middle,i)
       if(pivot .eq. 0._dkind) go to 999
...
900 if (w(middle,nrow) .ne. 0._dkind) return
999 iflag = 2
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    integer, intent(in)                               :: nroww,nrow
    integer, intent(in)                               :: nbandl,nbandu
    real(kind=dkind), dimension(nroww,nrow), intent(in) :: w
    real(kind=dkind), dimension(nrow), intent(inout)    :: b
